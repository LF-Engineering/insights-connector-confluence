...
./confluence --confluence-url='https://wiki.o-ran-sc.org' --confluence-debug=0 --confluence-es-url="${ESURL}" --confluence-stream="${STREAM}" $* 2>&1 | tee run3.log
#./confluence --confluence-url='https://wiki.tungsten.io' --confluence-debug=0 --confluence-es-url="${ESURL}" --confluence-stream="${STREAM}" $* 2>&1 | tee run4.log
#./confluence --confluence-url='https://finosfoundation.atlassian.net/wiki' --confluence-debug=1 --confluence-es-url="${ESURL}" --confluence-stream="${STREAM}" $* 2>&1 | tee run5.log
