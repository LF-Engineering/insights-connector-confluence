...
./confluence --confluence-url='https://wiki.riscv.org' --confluence-debug=1 --confluence-es-url="${ESURL}" --confluence-user="`cat ./secrets/user.encrypted.secret`" --confluence-token="`cat ./secrets/token.encrypted.secret`" --confluence-stream="${STREAM}" $* 2>&1 | tee run.log
#./confluence --confluence-url='https://wiki.lfai.foundation' --confluence-debug=1 --confluence-es-url="${ESURL}" --confluence-stream="${STREAM}" $* 2>&1 | tee run2.log
